        ...
        return {
            ...
            controller: ['$scope', '$timeout', function($scope, $timeout) {
                ...
                $scope.choose = function(x) {
                    $scope.showList = [], $scope.myObject = x, $scope.output = x, $scope.callback({
                        obj: x
                    }), $scope.inputBox = x[[$scope.search[0]]];
                }
                $scope.doSearch = function() {
                    ...
                    for (var i = 0; i < $scope.array.length; i++)
                        for (var j = 0; j < $scope.search.length; j++) String($scope.array[i][
                            [$scope.search[j]]
                        ]).toLowerCase().includes(param) && $scope.showList.push($scope.array[i]);
                }
                $scope.generateDisplay = function(a) {
                    for (var b = "", c = 0; c < $scope.display.length; c++) b += "<label>" + a[[$scope.display[c]]] + "</label> <br>";
